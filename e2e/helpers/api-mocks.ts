...
export async function installApiMocks(page: Page): Promise<CapturedRequest[]> {
  ...
  await page.route('**/api/**', async (route) => {
    ...
    if (url.pathname === '/api/transcriptions' && request.method() === 'POST') {
      await route.fulfill({ json: { data: { id: 'tr_demo', status: 'completed', created_at: TIMESTAMP, updated_at: TIMESTAMP, result: { channels: [{ transcript: 'Patient reports mild asthma.', confidence: 0.97 }] } } } });
      return;
    }
    ...
    if (url.pathname === '/api/codings' && request.method() === 'POST') {
      await route.fulfill({ json: { data: { id: 'coding_demo', status: 'completed', created_at: TIMESTAMP, updated_at: TIMESTAMP, result: { diagnoses: [{ id: 'd1', code: { coding: [{ system: 'ICD-10', code: 'J45.909', display: 'Unspecified asthma' }], text: 'Asthma' }, text_span: { start_char: 0, end_char: 6, text: 'asthma' } }], procedures: [] } } } });
      return;
    }
